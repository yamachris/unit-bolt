...
import { Edit2, X, Check, Camera, Heart } from "lucide-react";
...
export function ProfileEditor() {
  const { currentPlayer, updateProfile, opponentPlayer } = useGameStore();
  const [isEditing, setIsEditing] = useState(false);
  const [name, setName] = useState(currentPlayer.name);
  const [epithet, setEpithet] = useState(
    currentPlayer.profile.epithet || "Maître des Cartes",
  );
  ...
  return (
    <div className="bg-white/95 dark:bg-gray-800/95 rounded-xl p-4 shadow-lg backdrop-blur-sm">
      ...
      {/* Opponent Section - within same div */}
      {opponentPlayer && (
        <>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              <div className="w-12 h-12 rounded-full overflow-hidden">
                {opponentPlayer.profile?.avatar ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img
                    src={opponentPlayer.profile.avatar}
              ...
              <div className="absolute -top-1 -right-1 w-3 h-3 rounded-full border border-white dark:border-gray-800 bg-green-400"></div>
            </div>
            ...
            <div className="flex-1">
              <div>
                ...
                {opponentPlayer.profile?.epithet &&
                  opponentPlayer.profile.epithet.trim() !== "" && (
            ...
            <div className="text-right">
              <div className="flex items-center gap-1">
                ...
                <span className="font-semibold text-sm text-gray-900 dark:text-gray-100">
                  {opponentPlayer.health}/{opponentPlayer.maxHealth}
                </span>
              </div>
            </div>
          </div>
